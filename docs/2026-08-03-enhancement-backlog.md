...
| A16 | **Image parts land at the package root (`/media/image.png`), not `word/media/` where Word writes them.** Found 2026-08-03 by unpacking a document `ReplaceImage` produced. It is **legal and it resolves**: the relationship uses an absolute internal target, its `Id` matches the `r:embed`, the content type is registered, the embedded bytes are byte-identical to the input, and `OpenXmlValidator` passes. But no Word-authored `.docx` looks like this, and "unusual but legal" is what trips older readers and third-party consumers. **Investigated and not fixable at the call site:** switching from `AddNewPart<ImagePart>(contentType)` to the typed `AddImagePart(ImagePartType)` produces the *identical* location — measured, on `MainDocumentPart` and `HeaderPart` both, with the main part correctly at `/word/document.xml`. DocumentFormat.OpenXml 3.5.1 simply places `ImagePart` at the root. **Next step is evidence, not code:** open a generated document in Word and see whether the image renders. If it does, close this as a cosmetic difference and record that. If it does not, the fix is deeper than a method swap. |
