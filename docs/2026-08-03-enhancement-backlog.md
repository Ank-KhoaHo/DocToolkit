...
**D11 — no package icon.** Both packages showed nuget.org's default grey placeholder. Closed by a
128x128 PNG at `assets/icon.png`, shared by both packages and verified present inside both
`.nupkg` files rather than assumed from the `<PackageIcon>` property alone — that property without
a matching `Pack="true"` item builds clean and ships no icon. Recorded and closed the same day, so
it never spent time as an open item.

**A3 (reading slice) — XLSX bulk reading.** `SheetNames` and `ReadSheet` shipped in v0.6.0. Bulk
writing, formulas and CSV remain open under A3.
...
| A16 | **Closed 2026-08-04 — cosmetic, not a defect.** Image parts land at the package root (`/media/image.jpeg`) rather than `word/media/` where Word writes them. **Verified in Word:** four generated documents — an image in the body, one in a header *and* footer, one spliced inline mid-sentence, and a full invoice combining expanded rows, an image and scalars — all opened without a repair prompt and rendered correctly. The relationship uses an absolute internal target, which conforming readers must resolve, and Word does. Also recorded so nobody repeats it: switching from `AddNewPart<ImagePart>(contentType)` to the typed `AddImagePart(ImagePartType)` produces the **identical** location, measured on `MainDocumentPart` and `HeaderPart` both — the placement is DocumentFormat.OpenXml 3.5.1 behaviour, not something the call site controls. No change needed. |
...
| B10 | **Done 2026-08-03.** The timing assertion was removed after it failed a third time, on a PR that changed only a sample and a Markdown file. It turned out to be redundant, not merely flaky: `ConvertAsync` checks the token once on entry — which cannot fire when the token is still live at the call — and then hands it to `ParseBody`, with no check afterwards. So a token that never reached the parser would let the call run to completion and return a document, and the `ThrowsAnyAsync<OperationCanceledException>` assertion alone catches that. Verified by mutation: replacing `ParseBody(html, ct)` with `CancellationToken.None` still fails the test. Original text below. **`ConvertAsync_CancelsPartwayThroughALongParse` is timing-dependent and flakes.** It builds a 60,000-paragraph fixture, calls `cts.CancelAfter(150ms)`, and requires the token to land *inside* HtmlToOpenXml's work rather than at the entry guard. That holds only if the parse is still running at 150 ms, which depends on how loaded the runner is. Observed failing 2026-08-03 on a PR that changed nothing but YAML and Markdown, so it cost a red build for no reason. Second flaky test found in one day — the other compared two ClosedXML saves for byte equality — which together suggest a sweep for time- and machine-dependent assertions is worth doing rather than waiting for each to surface. **Fix shape:** assert cancellation is honoured without pinning *where* it is observed, e.g. cancel before the call and separately prove a mid-flight token is respected using a deterministic hook rather than a wall-clock race. |
