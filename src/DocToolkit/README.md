...
| **Headers and footers are one line each** | A header or footer is a single aligned line of text and page-number fields, set on `PageSetup`. Different odd and even pages are not supported, and neither is more than one header per document. |
