...
| **One page setup per document, except through `Merge`** | `PageSetup` applies to the whole document, and nothing lets you author a second section with different paper. `DocxEditor.Merge` is the one exception, and not by design: each merged document keeps its own page setup as its own section, so merging an A4 document with a Letter one produces a two-section file. Measured. |
| **`AddWatermark` marks one section per section the document *reports*, which is not one per `w:sectPr`** | Measured: a document produced by `DocxEditor.Merge` carries **two** `w:sectPr` elements in its body but reports a **single** section, so it receives one watermark rather than one per merged document — pages belonging to the later part may be unmarked. Watermark each document *before* merging if every page must carry one. |
| **`DocxEditor.Merge` resolves a style-id collision in favour of the FIRST document, silently** | Where two merged documents each define the same style id differently — a `Heading1` that is red in one and blue in the other — the first definition wins and the later content adopts it. No error is raised and no text is lost; only the appearance changes, which is what makes it easy to miss. Measured before the method was written. Give the documents distinct style ids, or merge documents that share a template. Aspose exposes an `ImportFormatMode` for this decision; this package deliberately does not, rather than adding a permanent option for it. |
