...
EXCLUDED = {
    ...
    "the two 'everything at a glance' Usage tours (root README's ~73-line block, "
        ...
        "That pass found ONE real defect, since fixed in both places it appeared (both root "
        "README ReplaceText calls): target-typed `new() { [\"{{customer}}\"] = ... }` against an "
        "IReadOnlyDictionary<string, string> parameter fails CS0144 (and CS0200 on the indexer) - "
        "not a narrative stand-in but a genuine compiler error, the same class of bug already "
        "corrected in readme-fill-rows, and fixed here the same way: spelled out as "
        "`new Dictionary<string, string> { ... }`. Everything else neither tour compiles as "
        "written is deliberate narrative, not a defect: the package tour declares a local named "
        "'report' three times in one scope (byte[], then ConversionResult<string>, then byte[] "
        "again) and reads 'xlsx' before assigning it; both tours reference variables the block "
        "itself never assigns (logoBytes, responseBody, remoteImages, xlsxStream, chartPngBytes "
        "in the package tour; lineItems, logoBytes, pptx, pngBytes in the root tour) standing in "
        "for 'data you already have', which is how a walkthrough reads and a script does not - "
        "each README now says so in one sentence immediately above its tour. Most of what either "
        "tour demonstrates already has its own compiled, assertion-backed region - the smaller "
        "sections in these same READMEs (FillRows, ReplaceImage, PdfEditor, PageSetup, remote "
        "images/opt-in, DI registration) or DocFX's DocumentationExamples.cs (the plain "
        "converters). Splitting the remainder would mean writing a couple dozen new [Fact]s whose "
        "only news is that an overload exists - duplicating coverage that is already real "
        "elsewhere - and would break the one place the API's breadth reads as a single connected "
        "story into a redundant restatement of it.",
}
